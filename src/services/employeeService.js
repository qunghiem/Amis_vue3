...
// convert dữ liệu trước khi gửi cho backend
const toBackend = (c) => ({
...
// chuyển đổi dữ liệu trước khi trả cho fe
const toFrontend = (e) => {
  const result = {
    ...e,
...
export const employeeService = {
  ...
  // thêm nhân viên
  async insert(c) {
    return (await api.post('/Employee', toBackend(c))).data
  },

  // Sửa nhân viên
  async update(c) {
    return (await api.put('/Employee', toBackend(c))).data
  },

  // Xóa nhân viên theo ID
  async deleteById(id) {
    return (await api.delete(`/Employee/${id}`)).data
  },

  // Xóa nhiều nhân viên theo ID
  async deleteMultiple(ids) {
    return (await api.delete('/Employee', { data: ids })).data
  },
}
